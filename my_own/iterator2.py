...
class DetRecordIter(mx.io.DataIter):
    ...
    def _get_batch(self):
        ...
        # label_draw = label[0,0,1:]
        # x0 = int(label_draw[0]*300)
        # y0 = int(label_draw[1]*300)
        # x1 = int(label_draw[2]*300)
        # y1 = int(label_draw[3]*300)
        # img_draw = self._batch.data[0].asnumpy()[0,0]
        # img_draw = img_draw.astype(np.uint8)
        # print(label_draw[0], x0)
        # cv2.rectangle(img_draw, (x0,y0), (x1, y1), (255), 3)
        # cv2.imshow("w", img_draw)
        # cv2.waitKey()
        #plt.imshow(self._batch.data[0].asnumpy()[0,0])
        #plt.show()
